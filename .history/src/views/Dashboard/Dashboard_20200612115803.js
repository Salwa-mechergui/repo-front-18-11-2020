...
function Dashboard(props) {
  ...
  const handleChange = async (id,v) => {
    let value = v ||  {id_customer(pin):11235,
saas_company_id(pin):1,
front_end_user_id(pin):11075,
first_name(pin):"Mohammed",
last_name(pin):"Ahmed12",
email(pin):"mohammed.test111@gmail.com",
phone_number(pin):"+21626859658",
default_passenger_id(pin):"32912"}
let idCustumer =v.id_customer ||1
    await props.searchUserbyid(1, idCustumer);
    await props.searchPassengers(idCustumer);
    await props.searchCompanies(1, idCustumer);
    await props.searchRides(1, idCustumer, "way_to");
    setSelected(true);
    setUserData(true);
